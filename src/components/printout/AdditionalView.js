...
const AdditionalView = ({ className, data }) => {
    ...
    return (
        <PrintBox header={"Additional"} className={className} boxSx={{justifyContent:"space-around"}}>
            {(rep || contact) ? <>
                <Box sx={{display:"flex", justifyContent:"center"}}>
                    {rep ? <>

                        <Typography variant='h3' sx={{margin:"5px"}}>{rep}</Typography>

                    </> : null}
                    {contact ? <>

                        <Typography variant="h3" sx={{margin:"5px"}}>{contact}</Typography>
