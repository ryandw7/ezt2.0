...
const PageBox = ({ children, header = {}, subHeader = "" }) => {
    ...
    return (
        <Box sx={{ height: `calc(100vh - 100px)`, maxHeight: `calc(100vh - 100px)`, width: "100vw", overflow: "hidden" }}>
            <Box sx={{ height: "50px", width: "100%", p: "0", backgroundColor: "#673AB7", textWrap:"nowrap" }}>
                <Typography sx={{ color: "white", fontSize: "2em", marginLeft: "5%", display:"inline" }}>{header}</Typography>
                {subHeader ? <>
                    <Typography sx={{color:"white", fontSize: "1.5em", display:"inline", marginLeft:"5px"}}>{subHeader}</Typography>
                </> : null}
            </Box>
